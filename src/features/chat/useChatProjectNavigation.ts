...
export function useChatProjectNavigation(): ChatProjectNavigationPort {
  ...
  return {
    ...
    setComposerText: (text) => value.portRef.current?.setComposerText(text),
    appendComposerText: (text) => value.portRef.current?.appendComposerText(text),
    addComposerAttachments: (attachments, textToAppend) =>
      value.portRef.current?.addComposerAttachments(attachments, textToAppend) ?? Promise.resolve(false),
    showChat: () => value.portRef.current?.showChat(),
    openArtifact: (artifactId) => value.portRef.current?.openArtifact(artifactId),
...
export function useRegisterChatProjectNavigation(port: ChatProjectNavigationPort): void {
  ...
  useEffect(() => {
    const registeredPort: ChatProjectNavigationPort = {
      ...
      setComposerText: (text) => latestPortRef.current.setComposerText(text),
      appendComposerText: (text) => latestPortRef.current.appendComposerText(text),
      addComposerAttachments: (attachments, textToAppend) =>
        latestPortRef.current.addComposerAttachments(attachments, textToAppend),
      showChat: () => latestPortRef.current.showChat(),
      openArtifact: (artifactId) => latestPortRef.current.openArtifact(artifactId),
