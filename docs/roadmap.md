...
- 单一正式签名者，证书 SHA-256 `F5746B0DC5BD3F6E640F693FDE171BD0CD87A919998CD6CA3F8F26748ABE6C02`；APK v2/v3 与 zipalign 通过，无 Android Debug 签名。该文件是本机 acceptance candidate，不是 GitHub Release 资产。
- 该 production candidate 只早于最终 `faceIDPermission: false` 的 iOS 元数据硬化。当前树随后重新完成了 Android 干净 prebuild/Release 构建及 Manifest 复核，但新构建由 Android Debug 证书签名；精确当前树的 production-signed APK 需要受保护签名环境，不能把 debug 包用于发布或覆盖安装。
...
- 需要 GitHub owner 在确认范围后运行正式 Actions 构建、上传资产、创建 immutable Release 和 Pages 下载闭环。当前未执行 push、tag、Release、生产密钥配置、协作者/保护规则变更。
...
当前状态：仓库已经定义 PR/Push 质量工作流，以及“精确 main tag -> owner draft -> main-only 隔离预检 -> 未签名构建 -> 正式 keystore 单签名 -> GitHub asset digest/uploader 校验 -> immutable Release -> Pages 公共更新清单与可信 `release.html` 下载页”的 CI 流程。Pages 只接受 owner 发布且由 Actions 上传资产的 Immutable Release，并在 APK 字节、GitHub digest 与对应校验文件全部匹配后公开下载页。当前公开 stable Latest 为 `v1.7.0`；`v1.4.0` 的 Release/3 个资产 attestation、证书指纹、Pages manifest/下载页、APK `HEAD` 与匿名完整 APK 字节验证是历史发布证据。M4 的发布工程部分已闭环，剩余门槛是最终 Actions APK 的连接真机、代表性设备矩阵和更广的真实服务商账号/媒体任务矩阵。
