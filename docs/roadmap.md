...
2026-07-10 远端验证：PR #1–#4 已合并；PR #2 修复首轮真实 Pages `pages: read` 集成缺口，PR #4 合并为 `4e079c09ff946ccfaa330bb2ff1e6b29115eb983`，其 head/merge Quality 与 merge-SHA Pages build/deploy 均成功。匿名请求仍只得到 v1.0.3 + `apk: null` 的 fail-closed 清单，`release.html` 与旧 debug APK 均为 404。main 已强制 PR、严格 Quality 与讨论解决，并叠加 owner-through-PR 更新规则；`v*` 只允许 owner 创建/更新且另有无 bypass 的禁止移动/删除规则。Actions 只允许 GitHub-owned、拒绝 verified third-party 并强制完整 SHA，Immutable Releases 已为未来版本启用，`android-release` 仍只允许 `main` 且 secrets 为 0。正式签名备份、v1.0.4 tag/Release/公开资产和真机/真实账号矩阵仍未完成。
