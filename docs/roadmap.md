...
`1.8.0` 已是公开 stable Latest。实现与验证记录以 [1.8.0 formal release checkpoint](./CONTINUATION_CHECKPOINT_2026-07-14_V1.8.md) 为准；tag/main peeled commit 为 `cafecfdbc74bc314239802ee121a938d4fa2c113`，正式发布已由 owner 授权完成。

### 1.8.0 验证快照（2026-07-14）
...
- `D:\EmbezzleStudio-Releases\v1.8.0-candidate\Embezzle-Studio-v1.8.0-candidate-release.apk`，152,673,746 bytes，SHA-256 `0DCB06FE4D8D2D018B78E1F6A7A684BCD66BB525682B622DA0399B289DFD3143`。
- 包名 `com.szdtzpj.embezzlestudio`，版本 `1.8.0` / versionCode `14`，minSdk/targetSdk `24/36`；有意保留 `RECORD_AUDIO`，无 `CAMERA`、`SYSTEM_ALERT_WINDOW`、`REQUEST_INSTALL_PACKAGES`、`USE_BIOMETRIC` 或 `USE_FINGERPRINT`。
- 单一正式签名者，证书 SHA-256 `F5746B0DC5BD3F6E640F693FDE171BD0CD87A919998CD6CA3F8F26748ABE6C02`；APK v2/v3 与 zipalign 通过，无 Android Debug 签名。该文件是发布前本机 acceptance candidate，不是公开资产；正式 Actions APK 见下方发布证据。
- 该 production candidate 只早于最终 `faceIDPermission: false` 的 iOS 元数据硬化；后续精确 tag/main 源码已由受保护 Android workflow 重建、生产签名并上传到 immutable Release。不能把本机 candidate 或 debug 重建包替代正式 Actions APK。

仅浏览器验证、尚未覆盖 Android 真机：

- 内置浏览器已在桌面视口及真实 `390×844` 视口覆盖首次配置、Chat、项目、成果/搜索、设置和 WebDAV/S3 表单；未观察到应用 console error。Web 上唯一提示是 Expo Notifications 不支持完整 push-token listener 的已知 warning。
- 这些结果只证明 Web/共享业务状态和导航路径，不能替代 Android 键盘与系统栏、系统分享、SAF、PDF/OCR、后台任务存活、媒体播放和性能测试。

仍需真实设备、真实账号或 GitHub 外部完成：

- `adb devices -l` 当前为空；需要在真实 Android 设备上安装最终 Actions APK，覆盖键盘/安全区、分享提供方、SAF 取消/失败/低空间、PDF/OCR、后台媒体长任务、通知、更多机型与压力矩阵。
- 需要用户自有 WebDAV/S3 账户验证 CAS/ETag、加密同步、冲突与恢复；需要真实服务商账号验证模型权限、计费/配额、长时媒体、上传/参考素材及错误矩阵。
- GitHub 发布边界已完成：主 Quality run `29384672597`、受保护 Android run `29384863423`、发布后 Pages run `29385766421` 均成功；正式 Release 为 [`v1.8.0`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.8.0)，APK 152,673,746 bytes，SHA-256 `d4164aec039e5d827fcab09f9805d760ca06c4e285d1720ef874a31c32391e0e`。公开 `release.html`、manifest 与 APK 下载均返回 HTTP 200，完整 Pages 字节与 Release 一致；未来 tag、签名 secrets 与生产 workflow 仍由 owner 控制。

P2 生物识别锁仍明确延期；本版本不新增 `expo-local-authentication`、指纹/Face ID 流程或相关权限。
...
`1.4.0` / code 10 的本机候选质量门已完成：41 个测试文件 / 749 个测试、TypeScript/ESLint、3,264-module Web export（7.4 MB）、Expo 依赖检查/Doctor 20/20、390×844 本地拦截浏览器批准/拒绝/取消路径、YAML/Bash/Action-SHA/diff/敏感边界检查均通过。干净 Android prebuild/未签名 Release、正式证书本机签名、aapt/apksigner/zipalign 复核通过；候选 `D:\EmbezzleStudio-Releases\v1.4.0-candidate\Embezzle-Studio-v1.4.0-candidate-release.apk` 为 97,518,039 字节，SHA-256 `683eb6e98efec3e301594e59c627b3698b410c2a58f841b3c3c3642b1a2a20ed`。

PR #15 的 head 为 `1176df7964712078d58c5eade50d781a8245d52e`，合并后的 `main`、tag `v1.4.0` 和 Release target 均为 `f83cea7fae36fcbaa0bff361fac2113c3edfb3d7`。PR Quality `29182946741`、main Quality `29183001171`、tag 前 Pages `29183001176`、production Android `29183097617`、发布后 Pages `29183525831` 均成功。正式 [`v1.4.0` Release](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.4.0) 发布时曾为 stable Latest；现在它是 immutable、非 prerelease 的历史 Release，3 个资产均由 `github-actions[bot]` 上传且 Release/asset attestations 全部通过。

正式 APK 存于 `D:\EmbezzleStudio-Releases\v1.4.0`，同为 97,518,039 字节，但 SHA-256 是 `c650e142e221821f8da91e37fefd76dad0e7ad94c0348a3d7749b69f14fc67eb`，与本机 candidate 明确不同。其包名/版本/code 为 `com.szdtzpj.embezzlestudio` / 1.4.0 / 10，min/target 24/36，`allowBackup=false`，有意 `RECORD_AUDIO`，无 CAMERA/`SYSTEM_ALERT_WINDOW`，单一正式 signer、证书 SHA-256 `F5746B0DC5BD3F6E640F693FDE171BD0CD87A919998CD6CA3F8F26748ABE6C02`，v2/v3 与 zipalign 通过。Pages manifest、`release.html`、APK `HEAD` 和完整下载均匿名 HTTP 200 且匹配；公网复核副本位于 `D:\EmbezzleStudio-Releases\v1.4.0-pages-public-verify-20260712-150424`。无连接真机或真实 OpenAI/MCP 账号验收；完整证据见 [v1.4 continuation checkpoint](./CONTINUATION_CHECKPOINT_2026-07-12_V1.4.md)。
...
当前状态：仓库已经定义并成功执行“精确 main tag -> owner draft -> main-only 隔离预检 -> 未签名构建 -> 正式 keystore 单签名 -> GitHub asset digest/uploader 校验 -> immutable Release -> Pages 公共更新清单与可信 `release.html` 下载页”的 CI 流程。当前公开 stable Latest 为 [`v1.8.0`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.8.0)；正式 APK SHA-256 为 `d4164aec039e5d827fcab09f9805d760ca06c4e285d1720ef874a31c32391e0e`，主 Quality/Android/Pages run 分别为 `29384672597`、`29384863423`、`29385766421`，三项公开页面均已匿名 HTTP 200。M4 的发布工程部分已闭环，剩余门槛是最终 Actions APK 的连接真机、代表性设备矩阵和更广的真实服务商账号/媒体任务矩阵。

当前仓库是个人账号拥有的公开仓库；GitHub 的个人仓库只有 owner 与 collaborator 权限层级，不能把 `BlueOcean223` 提升为第二个 owner。其权限已设为平台允许的最高 collaborator 级别 `write`，并获得 owner-gated main-update ruleset 的 `pull_request` bypass，因此可以在 required Quality 通过后合并 PR；main 仍禁止 direct update、删除和 force-push。release tag、正式签名 secrets、Environment 和生产 workflow actor gate 继续 owner-only，因此协作维护权限不等于正式发布所有权。
