...
This file is the authoritative handoff for the interrupted audit/fix session and its completed production continuation. Treat the current working tree as authoritative; do not reset, discard, or overwrite user-owned ignored files such as `.claude/settings.local.json` or `api_key.txt`.
...
The continuation reached the locally provable boundary on 2026-07-10, after which the user authorized production signing, GitHub Secrets, tag/Release creation, workflow execution, and publication. Stable [`v1.0.6`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.0.6) is now production-signed, immutable, Latest, attested, and available through the verified public Pages download chain. Historical interruption/evidence/action sections later in this file are retained only for traceability; this section supersedes their stale status.

### Second Android follow-up — released `1.0.6`

After the user confirmed that the four `1.0.5` runtime fixes worked on a real phone, two final UI problems remained: Android phones with a three-button navigation bar could cover the bottom of the model picker, and the Expo-template icon/splash plus three-dot pending animation looked unfinished. The released `1.0.6` / Android versionCode 6 follow-up contains:
...
Pre-publication local evidence for `1.0.6`:
...
Formal publication evidence for `v1.0.6`:

- PR [#10](https://github.com/szdtzpj/Embezzle-Studio/pull/10) merged the two mobile follow-up commits into `main` as `888db913c154fc60fdc7fa4b9de947be55ab10c0`. PR Quality run `29092001906`, exact merge-SHA Quality run `29092159078`, and the push-triggered Pages run `29092159111` all succeeded before the tag was created.
- Annotated tag `v1.0.6` resolves exactly to that merge SHA. Owner-authored empty Draft validation, clean prebuild/Gradle build, unsigned identity/SDK/permission checks, isolated production signing, certificate verification, three-asset digest/uploader checks, and immutable publication all passed in [Android run `29092367202`](https://github.com/szdtzpj/Embezzle-Studio/actions/runs/29092367202).
- The formal assets are downloaded under `D:\EmbezzleStudio-Releases\v1.0.6`. `Embezzle-Studio-v1.0.6-release.apk` is 96,805,335 bytes with SHA-256 `1a1fa2d5dc2bac2293994a92e0e65e7033bb4006082e503125d580c778d104f9`; package `com.szdtzpj.embezzlestudio`, version `1.0.6`/versionCode 6, minSdk 24, targetSdk 36; exactly one signer with production certificate SHA-256 `F5746B0DC5BD3F6E640F693FDE171BD0CD87A919998CD6CA3F8F26748ABE6C02`; v2/v3 and zipalign pass; no overlay/camera/microphone permission. The checksum file, signing report, GitHub asset digests, and `github-actions[bot]` uploaders all match. `gh release verify` and all three `verify-asset` checks succeed.
- [Pages run `29094337390`](https://github.com/szdtzpj/Embezzle-Studio/actions/runs/29094337390) succeeded. Anonymous HTTPS checks return schema 1/version 1.0.6, a trusted `release.html`, the expected download URL/size/digest, and a 96,805,335-byte APK whose recomputed SHA-256 exactly matches the immutable GitHub Release asset.

Current external/device boundary:

- The user confirmed on their Android phone that the four original `1.0.5` main paths are fixed and later authorized `v1.0.6` publication. This is user-reported device acceptance plus a release decision, not locally captured evidence for the final Actions-built APK.
...
- The GitHub/signing/Pages production boundary is complete for `v1.0.6`; no GitHub publication action remains for the user. Only independent device/runtime coverage and broader real-provider/media acceptance remain outside the completed release chain.
...
- The chat subtree remains mounted while Settings is visible; Settings mounts lazily once and is then reused. Video surfaces are disabled while Settings is active. Android replaces the heavier animated press/screen/message paths with lighter views, native image selection avoids the Base64 memory spike, and model candidates initially render in batches of 60 with an explicit load-more control.

Local evidence for this candidate:

- `npm.cmd run check`: passed — 15 test files, 249 tests; TypeScript and ESLint zero errors/warnings.
- `npm.cmd run build:web`: passed — 3131 modules, 6.9 MB main bundle. `npx.cmd expo-doctor`: 20/20. `npx.cmd expo install --check`: passed.
- A 390×844 Playwright run verified Chat and Settings, an actual uploaded image rendered as a square pending preview, and 20 consecutive Settings/Chat switches. The browser console had zero errors; only React Native Web deprecation warnings in the `shadow*` and `pointerEvents` categories remained.
- Clean Expo prebuild and the unsigned `assembleRelease` build passed. The unsigned output was then signed locally with the same production certificate used by `v1.0.4`, producing the acceptance-only candidate `D:\EmbezzleStudio-Releases\v1.0.5-candidate\Embezzle-Studio-v1.0.5-candidate-release.apk`.
  - size: 96,473,241 bytes; SHA-256 `c390a116a592773f23626ac6b63ace40a881e710e61318eedd196c6c0d6b8bc7`
  - package `com.szdtzpj.embezzlestudio`; version `1.0.5`; versionCode 5; minSdk 24; targetSdk 36
  - signer certificate SHA-256 `F5746B0DC5BD3F6E640F693FDE171BD0CD87A919998CD6CA3F8F26748ABE6C02`; APK Signature Schemes v2 and v3 pass; zipalign passes
...
- Android CI now uses `aapt` on the unsigned artifact to enforce package/version/minSdk/targetSdk and reject overlay/camera/microphone permissions before signing. Draft inspection is isolated in a short `contents: write` preflight; preflight, signing, and publication are constrained by the main-only `android-release` Environment, build remains read-only, signing has no repository-token permission, and checkout credentials are not persisted. Owner/ref gates precede each privileged boundary; the workflow rechecks the exact tag/main commit and all three assets before and after freezing the draft as immutable. All 16 official GitHub Action uses are pinned to GitHub-verified latest-stable full commit SHAs.
...
- `npm.cmd run check`: passed after release cleanup — 13 test files, 238 tests; TypeScript and ESLint zero errors/warnings.
...
- `npm.cmd run build:web`: passed after release cleanup — 3185 modules, 6.9 MB main bundle. `npx.cmd expo-doctor`: 20/20.
...
- `git diff --check` passed. All 3 workflow YAML files parsed, all 35 embedded Bash blocks passed Git Bash `bash -n`, and all 16 official Action uses passed full-SHA validation. A structured Vitest regression suite now asserts exact workflow triggers, permission scopes, main-only Environment gates, tag/main checks, read-only build checkout, v2 signing, three-stage ref verification, and Action SHA pins.
...
- Immutable Releases remains enabled (`enabled: true`, `enforced_by_owner: false`). Existing v1.0.0–v1.0.3 remain mutable and are not retroactively trusted; stable [`v1.0.6`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.0.6) is owner-published, non-prerelease, immutable, and Latest.
- Environment `android-release` has a custom deployment branch policy allowing only `main`, contains all five production signing Secret names, and has no reviewer because GitHub does not offer required Environment reviewers for this private personal repository without Enterprise. By user decision, `BlueOcean223` remains an explicitly trusted write collaborator; this is not equivalent to two-person approval.
- The stable PKCS12 signing identity and byte-identical backup are stored outside the repository under ACL-restricted `D:\EmbezzleStudio-Release-Signing`; the production certificate SHA-256 is `F5746B0DC5BD3F6E640F693FDE171BD0CD87A919998CD6CA3F8F26748ABE6C02`. Password/Base64 material was never printed or committed. This same-disk recovery directory is not claimed to be an independent encrypted offline backup.
- The first production run `29074005662` exposed that a read-only build token cannot see an owner-authored Draft. PR [#7](https://github.com/szdtzpj/Embezzle-Studio/pull/7) isolated Draft inspection in a minimal write-capable preflight and merged as workflow/main SHA `b70eea32440300eddd0000a9b8a5f3fa28679280`. Because no-bypass tag rules already locked annotated `v1.0.4`, the successful one-time preflight proved that the tag still resolved to application source commit `0062d16329989cdcbba1edad4ff8945176126feb`, remained an ancestor of main, and differed from that main only by `.github/workflows/android-apk.yml`. The exception was removed immediately after publication; future releases again require exact tag/main equality.
- Production Android run [`29074959109`](https://github.com/szdtzpj/Embezzle-Studio/actions/runs/29074959109) succeeded through preflight, clean Expo prebuild, unsigned Gradle build, `aapt` identity/SDK/permission checks, isolated signing, certificate verification, asset upload, and immutable publication. The Release contains exactly three `github-actions[bot]` assets in the uploaded state.
- The formal APK downloaded to `D:\EmbezzleStudio-Releases\v1.0.4\Embezzle-Studio-v1.0.4-release.apk` is 93,087,208 bytes with SHA-256 `187f4a90daed7c7d05d423890419d1c4fe1d705674bf1d4955075c8d725b63f0`; package `com.szdtzpj.embezzlestudio`, version `1.0.4`, versionCode `4`, minSdk `24`, targetSdk `36`; no overlay/camera/microphone permissions; exactly one non-debug signer with the expected production fingerprint; APK Signature Schemes v2 and v3 true; zipalign succeeds. The checksum file, signing report, and all three GitHub asset digests match local bytes.
- Official `gh release verify v1.0.4` loaded the GitHub release attestation and succeeded. `gh release verify-asset` also succeeded independently for the APK, checksum, and signing report.
- Production Pages run [`29076831325`](https://github.com/szdtzpj/Embezzle-Studio/actions/runs/29076831325) completed build and deploy. Anonymous HTTPS verification returned schema 1/version 1.0.4 with a non-null trusted APK, `releaseUrl` pointing to `release.html`, a 200 download page containing the full digest, and a 200 public APK download whose 93,087,208 bytes hash exactly matched the GitHub Release APK.
- PR [#10](https://github.com/szdtzpj/Embezzle-Studio/pull/10) merged the `1.0.6` mobile/media/brand follow-up as `888db913c154fc60fdc7fa4b9de947be55ab10c0`; PR Quality `29092001906`, exact merge-SHA Quality `29092159078`, and pre-tag push Pages `29092159111` succeeded. Annotated `v1.0.6` and remote `main` both resolved to that exact commit at release freeze.
- Production Android run [`29092367202`](https://github.com/szdtzpj/Embezzle-Studio/actions/runs/29092367202) completed clean build, unsigned identity/permission validation, isolated signing, exact asset verification, and immutable Latest publication. The formal APK under `D:\EmbezzleStudio-Releases\v1.0.6` is 96,805,335 bytes with SHA-256 `1a1fa2d5dc2bac2293994a92e0e65e7033bb4006082e503125d580c778d104f9`; Release attestation and all three downloaded `verify-asset` checks pass, and independent `aapt`/`apksigner`/zipalign verification confirms version 1.0.6/code 6, minSdk 24/targetSdk 36, the expected single production certificate, and no overlay/camera/microphone permission.
- Production Pages run [`29094337390`](https://github.com/szdtzpj/Embezzle-Studio/actions/runs/29094337390) completed build/deploy from the successful Android workflow. The public manifest, trusted `release.html`, HEAD metadata, and an anonymous full APK download all match the immutable Release's name, 96,805,335-byte size, and SHA-256.
...
- Low-cost live discovery and text requests were verified against supplied Volcengine Ark, Alibaba Bailian, and third-party OpenAI-compatible accounts. A real OpenAI account, broad entitlement/billing coverage, long-running generation, media upload, and the complete provider error matrix remain unverified.
- Browser verification is complete for the exported Web artifact; there is no remaining “browser unavailable” gap.
- Production signing, GitHub Release/attestation, and the valid `release.html`/public APK path are proved remotely and by downloaded-byte verification. Only device/runtime and broader real-provider acceptance remain outside local automation.
...
At that historical interruption, the change was **not yet fully closed**:
...
The GitHub/signing production boundary is complete. These are the remaining user/device/service boundaries:

- `BlueOcean223` is intentionally retained as a trusted write collaborator. A future move to an organization/Enterprise plan is required if two-person Environment approval or a more granular collaborator role becomes mandatory.
- The D-drive recovery directory is ACL-restricted and contains two verified identical PKCS12 copies, but it is not independent removable/offline media and is not documented as BitLocker-encrypted. Creating an independently stored encrypted copy remains a security recommendation rather than an uncompleted release action.
- The formal Actions-built `v1.0.6` APK was not installed through a device connected to this session. The user had already confirmed IME avoidance, Seedance preview/download, image preview sizing, and Settings/Chat switching on the local production-signed candidate and later authorized publication; independent coverage of the new system inset, launcher/themed masks, splash, native animation, error paths, and sustained stress remains recommended.
- Existing `v1.0.3` and `v1.0.4-debug.*` installations use the Android Debug certificate. Export important data, uninstall the debug build, and install the current production `v1.0.6`; an in-place signature-changing upgrade is impossible.
- Run broader real-account acceptance for OpenAI, long-running image/video tasks, media upload/reference flows, billing/entitlement differences, and provider-specific error shapes. Kimi remains blocked by the supplied Bailian account reporting that the product is not activated.
...
- Current release-source validation: `npm.cmd run check` passed 15 files / 252 tests, `npm.cmd run build:web` passed (3137 modules, 6.9 MB main bundle), Expo Doctor passed 20/20, all 3 workflow YAML files parsed, all 35 embedded Bash blocks passed `bash -n`, all 16 Action uses remained GitHub-owned full SHAs, and `git diff --check` passed.
- Immutable debug prereleases remain historical test artifacts and use the Android Debug certificate. Stable Latest is now the production-signed immutable [`v1.0.6`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.0.6); the update chain deliberately selects it instead of any prerelease.
