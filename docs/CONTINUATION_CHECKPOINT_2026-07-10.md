...
- Immutable Releases remains enabled (`enabled: true`, `enforced_by_owner: false`). Existing v1.0.0–v1.0.3 remain mutable and are not retroactively trusted; stable [`v1.0.4`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.0.4) is owner-published, non-prerelease, immutable, and Latest.
...
- Install the production APK on a representative Android device/emulator and test launch, permission prompts, attachment picker, sharing/export, back handling, update-link handoff, and persistence. No device was connected during this session.
- Existing `v1.0.3` and `v1.0.4-debug.*` installations use the Android Debug certificate. Export important data, uninstall the debug build, and install production `v1.0.4`; an in-place signature-changing upgrade is impossible.
...
- Current source validation after release cleanup: `npm.cmd run check` passed 13 files / 238 tests, `npm.cmd run build:web` passed (3185 modules, 6.9 MB main bundle), Expo Doctor passed 20/20, all 3 workflow YAML files parsed, all 35 embedded Bash blocks passed `bash -n`, all 16 Action uses remained GitHub-owned full SHAs, and `git diff --check` passed.
- Immutable debug prereleases remain historical test artifacts and use the Android Debug certificate. Stable Latest is now the production-signed immutable [`v1.0.4`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.0.4); the update chain deliberately selects it instead of any prerelease.
