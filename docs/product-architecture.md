...
4. Local workspace and cost safety
   - Workspace schema v7 with first-run setup, simple/advanced mode, draft and backup recovery, and project/artifact/favorite/tag workspaces.
   ...
   - Device-local cleanup journaling, tombstones, orphan-media reclamation, and persisted Android media-task outbox/notification state; no server worker or cloud job database is required.
   - System-share intake and document import are bounded and failure-safe: caption/stream batches roll back atomically, share caches are reclaimed, PDF page IDs are stable and deduplicated, and mixed-PDF OCR remains an explicit placeholder until local or user-authorized provider OCR is selected.
...
flowchart TD
    ...
    Workspace --> Lifecycle["Cleanup Journal + Tombstones + Media Outbox"]
    ...
    UI --> ShareIntake["System Share Intake + Atomic Cache Rollback"]
...
The root `App.tsx` is now a small, explicit composition root:
...
- `WorkspaceSession` is the only writable Workspace State owner and controls boot, read-only, deferred/required durability, background/unmount flush, and replacement transactions. `WorkspaceSessionProvider` constructs the production-owned session without starting work during render, begins boot and lifecycle subscription from a committed effect, and disposes it on unmount. Selector consumers subscribe to stable full Workspace/status snapshots before deriving results; complete session access is kept behind an explicit internal context module rather than the public Provider surface.
...
- Chat's internal implementation is split by invariant-bearing change axis rather than by arbitrary line targets:
  ...
  - `ChatUsageLedger` owns cost-plan authorization, post-confirmation replacement revalidation, attempt insertion/pruning, required durability, removal of a newly started attempt when its visible-message commit fails, and unknown-cost completion semantics.
...
At the public module boundaries, the Provider surface no longer exports a generic Workspace session/mutation hook and no application-wide command bus remains. Code that needs the complete session imports the explicit internal seam; other consumers use selectors, status, or narrow feature capabilities. `ChatPane` remains the page-level coordinator for navigation, composer state, conversation intent preparation, Workbench entry, and the high-level React tree; independent request, ledger, voice, task, and presentation ownership lives in the dedicated modules above. The follow-up deliberately rejected a catch-all `useChatPaneModel`, a broad internal context, a mirrored mega-props view, and mechanical one-file-per-style-category splitting.

The PR #24 repair pass in the current tree:
...
- removes closed Settings from the Web layout while keeping the native pane mounted, replays an already-open Settings destination (including a specific provider model target), and exposes Projects capabilities through a small public barrel;
- blocks project/conversation request-context changes while a provider request owns the history lock, and refreshes project instructions across every empty conversation when a project changes or is activated;
- rolls back a persisted `started` usage attempt before releasing the Chat lease when visible request messages cannot be committed, and prevents unpersisted transcription/speech attempts from being misclassified as provider failures;
- blocks conversation branching while a request is active, closes transient Chat menus before Settings opens, and surfaces a ready-phase Workspace save failure in the root status UI;
- keeps Workspace construction render-pure, makes boot own the idempotent lifecycle subscription, and applies arbitrary selectors only after subscribing to the stable complete Workspace snapshot.

Exact source-line, test-count, and bundle-module totals are intentionally not frozen in this architecture overview because they apply only to the source state that produced them. The earlier recorded browser smoke covered Chat boot, Settings open/back, Projects drawer, bounded local search, and drawer close with zero console errors; current checkpoint, CI, browser, and device evidence must be consulted separately for post-repair acceptance.
...
- `ProjectKnowledgeSource`: project-scoped local reference text authored manually, captured from a message/artifact snapshot, or imported through the bounded text/HTML/Office/PDF/image document-import pipeline.
...
## Durable Workspace and Background Media Boundary

- Workspace schema v7 is the durable local contract. Boot performs one-time migration and normalization before exposing the ready phase; first-run setup, simple/advanced mode, draft recovery, and backup replacement are explicit state transitions rather than render-time side effects.
- Destructive conversation/message changes use a device-local cleanup journal. A clean workspace snapshot is committed first, then tombstones and task references are recorded; journal drain removes stale outbox entries and orphaned app-owned media only after the new snapshot is durable. The journal is device state and is excluded from authenticated backups and optional cloud-sync payloads.
- Android background media work is represented by durable conversation/task records plus a per-entry outbox and local notification state. Outbox merges are monotonic within a task lifecycle: terminal states never regress to pending, newer metadata wins, notification `sent` is not downgraded, and each write is read-back verified. The task center is a local projection, not a hosted worker or cloud job database.
- System-share intake merges text captions and stream-backed attachments in bounded batches. Cache copies are committed atomically with rollback on any failure, `cacheDir/expo-sharing` is reclaimed after intake, and URL captions remain opaque text; the app does not fetch a URL merely because it appears in shared text.
- Document import assigns stable page-number IDs, deduplicates repeated PDF records, adds only missing mixed-PDF OCR placeholders, updates the exact OCR page on completion, and cleans picker cache files when parse/read fails. Unsupported formats remain rejected rather than being accepted through a renamed extension.
- P2 biometric lock is intentionally outside this architecture. Biometric/fingerprint permissions are blocked as a release hardening gate; no biometric API, prompt, or unlock flow is implied by schema v7 or the current P0/P1 scope.
...
- External backups are authenticated encrypted files, but secrets, media, `providerUsageEvents`, and local MCP activity summaries are removed before encryption. Internal workspace schema v7 migrates v6/v5/v4/v3/v2, stores provider/MCP/sync secrets separately, preserves the current device's attempt ledger on import, and strictly normalizes projects, branches, artifacts, project reference sources, MCP activity metadata, cloud-sync settings, and cost settings. Backup import always restores every remote MCP entry as disabled; internal loading also disables any entry not bound to the exact official OpenAI provider kind and canonical endpoint.
...
- Project-source import accepts an explicit allowlist of plain text/code, HTML, DOCX, XLSX, PPTX, PDF, and image inputs with byte, entry, page, and character limits. Office files are parsed as inert XML text; Android PDF text extraction and ML Kit OCR are native, while Web and module-less builds keep an explicit pending-OCR draft. Legacy binary Office, OpenDocument, archives, APKs, and executables remain unsupported.
...
- These local capabilities require no Embezzle Studio API, server, telemetry, vector store, or app-owned quota. Optional sync uses only a user-owned WebDAV or S3-compatible endpoint and never uploads media, API keys, sync credentials, or `providerUsageEvents`; it is not an Embezzle-operated sync service. If selected context is sent to a model, the user's configured provider endpoint, key, entitlement, quota, and billing remain authoritative.

The original `1.3.0` artifact contract and the current `1.8.0` import boundaries are recorded in [Local Knowledge and Artifact Workbench](./local-knowledge-workbench.md).

## User-owned Cloud Sync Boundary

- Sync is an optional client-only transport for an authenticated encrypted workspace snapshot. The app supports WebDAV Basic-auth over HTTPS and S3-compatible path-style SigV4; it does not create a server, bucket, WebDAV collection, worker, push channel, or quota.
- The remote object contains only the encrypted snapshot and a small manifest/history. Media bytes, provider API keys, MCP authorization, WebDAV/S3 credentials, and the device-local `providerUsageEvents` ledger stay on the device. A user must supply an already-created WebDAV directory or S3 bucket/prefix; the client does not silently create remote hierarchy. HTTP is accepted only for loopback development endpoints; HTTPS private/self-hosted destinations are an explicit user trust decision.
- Every read/write is bounded to 10 MiB objects and a 64 KiB manifest. ETag/`If-Match`/`If-None-Match` compare-and-swap is required; if the transport cannot prove conditional-write behavior, sync fails closed instead of overwriting a remote snapshot.
- First sync, unchanged, push, pull, remote-missing, and conflict are separate outcomes. A conflict never retries implicitly: the user must explicitly keep the local or remote snapshot, and replacement is committed locally only after the encrypted snapshot has been verified and durably written.
- Web uses session-scoped sync credentials and native platforms use SecureStore. Sync is not a backup substitute: users should retain an authenticated encrypted export and understand that remote storage provider retention, access control, billing, and audit logs remain outside Embezzle Studio.
...
- Stable [`v1.8.0`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.8.0) is the current immutable, non-prerelease Latest Release with exactly 3 attested assets. Its formal APK is `152,673,746` bytes with SHA-256 `d4164aec039e5d827fcab09f9805d760ca06c4e285d1720ef874a31c32391e0e`; it identifies as `com.szdtzpj.embezzlestudio` version `1.8.0` / code `14`, minSdk `24`, targetSdk `36`, `allowBackup=false`, `adjustResize`, and intentional `RECORD_AUDIO`, with no camera, overlay, install-packages, biometric, or fingerprint permission. It has exactly one expected production signer with v2/v3 and zipalign verification. The Release APK, checksum, and signer report are mirrored by the Pages manifest and `release.html`, and an anonymous full Pages download matches the Release byte-for-byte.
- The P0/P1 working tree represented by peeled tag/main commit `cafecfdbc74bc314239802ee121a938d4fa2c113` is now published through protected Android run `29384863423` and post-release Pages run `29385766421`. Local signed candidates remain pre-publication validation evidence only. P2 biometric lock is intentionally deferred and must not be inferred from the released scope.
- This is a personal public repository, so GitHub exposes owner and collaborator roles rather than a second co-owner role. `BlueOcean223` has the maximum collaborator permission (`write`) plus a `pull_request` bypass on the owner-gated main-update ruleset: they can merge PRs after the required Quality check succeeds, but cannot assume owner identity. Release-tag creation, signing secrets, and production publication remain owner-only.

The remaining release bullets in this section are historical snapshots from earlier stable versions; they are retained as provenance and must not be read as current `main` or current-release evidence.

## Current Verification Boundary (v1.8.0 P0/P1)

- The active truth is [the v1.8.0 formal release checkpoint](./CONTINUATION_CHECKPOINT_2026-07-14_V1.8.md). It preserves the 2026-07-14 local evidence and records the authorized 2026-07-15 tag, immutable Release, protected signing run, and public Pages closure. The working tree remains authoritative; no reset, checkout overwrite, collaborator-permission change, or remote-protection change was used for the release continuation.
- Completed local scope includes workspace schema v7 and first-run/recovery flows; projects, artifacts, favorites/tags, branches, and bounded cross-project search; Android background media tasks, persisted outbox, and local notifications; cleanup journaling, tombstones, monotonic task-state merging, and orphan-media reclamation; atomic system-share intake and cache cleanup; stable/deduplicated PDF import with explicit mixed-PDF OCR placeholders; and optional fail-closed WebDAV/S3 sync that keeps keys, credentials, media, and the local usage ledger on-device. P2 biometric lock remains deferred.
- `npm.cmd run check` passes 76 test files / 1,018 tests with clean TypeScript and ESLint. Web export passes at 3,483 modules with an approximately 8.2 MB main bundle. `npx.cmd expo install --check` passes and Expo Doctor is 20/20.
- Three workflow YAML files parse, 36 embedded Bash blocks pass `bash -n`, all 16 action references use full SHA pins, `git diff --check` passes, and the reverse check for `patches/expo-sharing+57.0.3.patch` passes. `npm audit --omit=dev --audit-level=high` exits 0; twelve moderate `uuid -> xcode -> @expo/config-plugins` findings remain and no incompatible `npm audit fix --force` was applied.
- In-app Browser smoke passes at the normal desktop viewport and a true 390x844 viewport for first-run configuration, Chat, Projects, artifacts/search, Settings, and WebDAV/S3 forms with no app console errors. The only observed warning is the documented Expo Notifications web limitation around push-token listeners; this is not Android-device evidence.
- Clean Android prebuild and offline Release assembly pass. The production-signed local acceptance candidate is `D:\EmbezzleStudio-Releases\v1.8.0-candidate\Embezzle-Studio-v1.8.0-candidate-release.apk`, 152,673,746 bytes, SHA-256 `0DCB06FE4D8D2D018B78E1F6A7A684BCD66BB525682B622DA0399B289DFD3143`. It identifies as `com.szdtzpj.embezzlestudio` version `1.8.0` / code `14`, min/target SDK `24/36`, `allowBackup=false`, `adjustResize`, intentional `RECORD_AUDIO`, and no camera, overlay, install-packages, biometric, or fingerprint permission. It has exactly one production signer (`F5746B0DC5BD3F6E640F693FDE171BD0CD87A919998CD6CA3F8F26748ABE6C02`); v2/v3 and zipalign pass. This candidate and the later debug-signed manifest rebuild remain historical pre-publication evidence. The exact tagged source was independently rebuilt and signed by the protected workflow; its formal APK under `D:\EmbezzleStudio-Releases\v1.8.0` has SHA-256 `d4164aec039e5d827fcab09f9805d760ca06c4e285d1720ef874a31c32391e0e` and is the only public release artifact.
- `adb devices -l` is empty for this run. Real-device install/launch, native keyboard/system-bar variants, share providers, PDF/OCR/SAF cancellation and low-space behavior, background-task survival, performance/stress, real WebDAV/S3 CAS/conflict behavior, broad real-provider entitlement/billing, and long-running media acceptance remain external boundaries. This publication is complete; future production tags, signing secrets, and release workflows remain owner-controlled actions.

## Historical Verification Provenance

- Release PR #15 used head `1176df7964712078d58c5eade50d781a8245d52e` and merged into `main` as `f83cea7fae36fcbaa0bff361fac2113c3edfb3d7`. Tag `v1.4.0`, the Release target, and public stable metadata all resolve to that exact commit at version `1.4.0` / code 10.
...
- PR Quality `29182946741`, `main` Quality `29183001171`, pre-tag Pages `29183001176`, production Android `29183097617`, and post-release Pages `29183525831` all succeeded. The stable Release is Latest, immutable, non-prerelease, has exactly 3 `github-actions[bot]`-uploaded assets, and passes its Release attestation plus all 3 asset attestations.
- The formal `Embezzle-Studio-v1.4.0-release.apk` is 97,518,039 bytes with SHA-256 `c650e142e221821f8da91e37fefd76dad0e7ad94c0348a3d7749b69f14fc67eb`; it is distinct from the same-size local candidate. It independently identifies as `com.szdtzpj.embezzlestudio` version `1.4.0`/code 10, minSdk 24/targetSdk 36, `allowBackup=false`, intentional `RECORD_AUDIO`, and no CAMERA or `SYSTEM_ALERT_WINDOW`. It has exactly one signer with production certificate SHA-256 `F5746B0DC5BD3F6E640F693FDE171BD0CD87A919998CD6CA3F8F26748ABE6C02`; v2/v3 and zipalign pass.
- Formal assets are stored under `D:\EmbezzleStudio-Releases\v1.4.0`. The Pages manifest, `release.html`, APK `HEAD`, and full APK return anonymous HTTP 200 and match the Release; the public verification copy under `D:\EmbezzleStudio-Releases\v1.4.0-pages-public-verify-20260712-150424` matches the formal size and hash.
...
- PR Quality run `29176034579`, `main` Quality run `29176125303`, initial Pages run `29176125307`, production Android run `29176245049`, and post-release Pages run `29176763721` all succeeded. The historical `v1.3.0` Release was Latest when published, is immutable and non-prerelease, and contains exactly 3 assets; its Release attestation and all 3 asset attestations pass.
...
- `adb devices -l` is still empty. No connected-device test of the formal v1.8.0 Actions APK, additional-device matrix, real-provider-account search/MCP behavior, cloud-provider CAS behavior, or billing acceptance is claimed.
...
- The device-local cleanup journal, outbox, notification state, and tombstones are operational recovery state rather than workspace content; they are not included in encrypted backup or optional WebDAV/S3 sync. The formal v1.8.0 APK's `allowBackup=false` gate prevents Android automatic backup from becoming an unreviewed second copy of workspace or usage state.
- Android sets `allowBackup: false` so workspace conversations and the local attempt ledger are not eligible for Android/Google automatic app backup; cross-device migration should use the explicit authenticated encrypted export. The formal Actions APKs for `v1.3.0`, `v1.4.0`, `v1.7.0`, and current `v1.8.0` independently verify `android:allowBackup="false"`; older candidate evidence remains historical.
