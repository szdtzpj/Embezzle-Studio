...
- Stable [`v1.7.0`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.7.0) remains the current immutable, non-prerelease Latest Release with exactly 3 attested assets. Its formal APK is `97,731,031` bytes with SHA-256 `638fd7cbe378d76476f1147f3cd7fbbe491e8dfb048553491387ba77319938bf`; it identifies as `com.szdtzpj.embezzlestudio` version `1.7.0` / code `13`, minSdk `24`, targetSdk `36`, `allowBackup=false`, and has one production signer with v2/v3 and zipalign verification. The formal Release APK, checksum, and signer report are mirrored by the Pages manifest and `release.html`, and an anonymous full Pages download matches the Release byte-for-byte.
- The current working tree is an unpublished `1.8.0` / code `14` P0/P1 development build. It is not represented by a tag, GitHub Release, or Actions APK; any local signed candidate remains validation evidence only. P2 biometric lock is intentionally deferred and must not be inferred from the current scope.
...
- The active local truth is [the v1.8.0 continuation checkpoint](./CONTINUATION_CHECKPOINT_2026-07-14_V1.8.md). It records the completed P0/P1 scope, exact command evidence, the local Android candidate, and the external/manual boundary. The working tree remains authoritative; no reset, checkout, overwrite, push, tag, GitHub Release, signing-secret configuration, or remote-protection change is implied by this document.
...
- `npm.cmd run check` passes 76 test files / 1,012 tests with clean TypeScript and ESLint. Web export passes at 3,483 modules with an approximately 8.2 MB main bundle. `npx.cmd expo install --check` passes and Expo Doctor is 20/20.
...
- `adb devices -l` is still empty. No connected-device test of the formal v1.7.0 Actions APK or this unreleased local repair, additional-device matrix, real-provider-account search/MCP behavior, cloud-provider CAS behavior, or billing acceptance is claimed.
...
- The device-local cleanup journal, outbox, notification state, and tombstones are operational recovery state rather than workspace content; they are not included in encrypted backup or optional WebDAV/S3 sync. The v1.8.0 candidate's `allowBackup=false` gate prevents Android automatic backup from becoming an unreviewed second copy of workspace or usage state.
- Android sets `allowBackup: false` so workspace conversations and the local attempt ledger are not eligible for Android/Google automatic app backup; cross-device migration should use the explicit authenticated encrypted export. The formal Actions APKs for `v1.3.0`, `v1.4.0`, and current `v1.7.0` independently verify `android:allowBackup="false"`; older candidate evidence remains historical.
