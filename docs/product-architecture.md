...
- Local development metadata is now version `1.3.0` / code 9. It is not tagged or public, and stable Latest remains `v1.0.6`.
...
- `adb devices -l` is empty. No final Android-device, real-provider-account, or billing result is claimed. No push, tag, upload, GitHub Release, Pages update, or public `1.3.0` APK has occurred.
...
- Android sets `allowBackup: false` so workspace conversations and the local attempt ledger are not eligible for Android/Google automatic app backup; cross-device migration should use the explicit authenticated encrypted export. The historical clean `1.2.0` package and the final local `1.3.0` generated Manifest/packaged candidate independently verify `android:allowBackup="false"`.
