...
- Stable [`v1.4.0`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.4.0) is the current immutable, non-prerelease Latest Release with exactly 3 attested assets. Previous stable `v1.3.0` and `v1.0.6` remain historical release evidence.
...
- `adb devices -l` is still empty. No connected-device test of the local v1.4 candidate or final v1.4 Actions APK, additional-device matrix, real-provider-account MCP behavior, or billing acceptance is claimed.
...
- Android sets `allowBackup: false` so workspace conversations and the local attempt ledger are not eligible for Android/Google automatic app backup; cross-device migration should use the explicit authenticated encrypted export. The historical clean `1.2.0` package plus the local candidate and Actions-built formal APKs for `v1.3.0` and `v1.4.0` independently verify `android:allowBackup="false"`.
