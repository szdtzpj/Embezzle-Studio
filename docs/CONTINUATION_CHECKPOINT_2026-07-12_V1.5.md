# Continuation Checkpoint — 2026-07-12 (`1.5.0` pre-publication)
...
- The current release-preparation work started from that exact synchronized `origin/main` commit and carries application metadata `1.5.0`, Android versionCode `11`, plus the audit fixes below.
- PR #12 is the collaborator-source integration point. It is not a `v1.5.0` release PR, tag, GitHub Release, production Android run, or published APK.
- The public stable, immutable, non-prerelease Latest Release remains [`v1.4.0`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.4.0) / Android versionCode 10. The D-drive `1.5.0` APK documented here is local pre-publication evidence only.
...
- The complete Web, Expo, dependency, workflow, diff, secret-boundary, and Android gates are green for the current worktree; any later source change invalidates this candidate evidence.

## Local Android candidate status
...
- The final local candidate must never be uploaded as the formal GitHub artifact. The protected workflow must rebuild the release from the exact tagged `main` commit, and its bytes/hash may differ even when package metadata and signing identity match.

## Not yet verified

- `adb devices -l` is empty. No installation, upgrade, system/light/dark theme switching, provider enable/disable cleanup, long-dialog scrolling, Settings Back flow, process-death recovery, or sustained Chat/Settings stress result is claimed for a connected Android device in this checkpoint.
...
- No `v1.5.0` release PR merge, tag, draft Release, production Android workflow, immutable Release attestation, Pages update, or public APK byte verification is claimed here.

## Remaining release boundary

1. Re-run `git diff --check` and the complete local gate after any source change, then commit and push the release-preparation branch intentionally.
2. Open and merge a release PR only after its required Quality check succeeds; then verify the push-triggered `main` Quality and Pages runs against the exact merge commit.
3. Freeze other release changes, create `v1.5.0` from the exact current `origin/main`, and have the repository owner create the reviewed empty non-prerelease Draft Release.
4. Run the protected production Android workflow from `main`. It must rebuild, verify, sign, upload exactly three assets, publish the immutable Latest Release, and pass Release/asset attestations.
5. Verify the post-release Pages manifest and `release.html`, anonymously download the full public APK, and compare its size/SHA-256 plus `aapt`, `apksigner`, and zipalign results with the Release metadata before ending the release freeze.
