...
describe('release metadata', () => {
  it('keeps every v1.7.0 version source synchronized with Android code 13', async () => {
    ...
    expect(expo.version).toBe('1.7.0');
    expect(expo.android?.versionCode).toBe(13);
    expect(packageJson.version).toBe('1.7.0');
    expect(packageLock.version).toBe('1.7.0');
    expect(lockPackages['']?.version).toBe('1.7.0');
    expect(appInfo.version).toBe('1.7.0');
  });
});
