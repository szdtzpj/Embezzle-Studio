...
describe('release metadata', () => {
  it('keeps every v1.8.0 version source synchronized with Android code 14', async () => {
    ...
    expect(expo.version).toBe('1.8.0');
    expect(expo.android?.versionCode).toBe(14);
    expect(packageJson.version).toBe('1.8.0');
    expect(packageLock.version).toBe('1.8.0');
    expect(lockPackages['']?.version).toBe('1.8.0');
    expect(appInfo.version).toBe('1.8.0');
  });
});
