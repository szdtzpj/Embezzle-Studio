...
The current stable Latest is [`v1.3.0`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.3.0) / Android versionCode 9. It is immutable, non-prerelease, and contains exactly 3 formal assets. The existing local `1.2.0` candidate remains historical evidence for the previous source tree, while the local `1.3.0` candidate must also be distinguished from the Actions-built formal APK.
...
The local `1.4.0` / code 10 candidate gate passes: 41 test files / 749 tests, TypeScript/ESLint, a 3,264-module Web export (7.4 MB), Expo Doctor 20/20, a 390×844 locally intercepted browser MCP approve/deny/cancel flow, YAML/Bash/Action-SHA/diff/secret boundaries, clean Android prebuild and unsigned Release assembly, local production signing, plus aapt/apksigner/zipalign verification. `D:\EmbezzleStudio-Releases\v1.4.0-candidate\Embezzle-Studio-v1.4.0-candidate-release.apk` is 97,518,039 bytes with SHA-256 `683eb6e98efec3e301594e59c627b3698b410c2a58f841b3c3c3642b1a2a20ed`; it identifies as `com.szdtzpj.embezzlestudio` 1.4.0/code 10 with min/target 24/36, `allowBackup=false`, no CAMERA/overlay permission, one expected production signer, v2/v3, and valid alignment. It has not been pushed, tagged, uploaded, or published and has no connected-device or real OpenAI/MCP-account acceptance. See the [`1.4.0` continuation checkpoint](./docs/CONTINUATION_CHECKPOINT_2026-07-12_V1.4.md) for the complete evidence and external boundary.
