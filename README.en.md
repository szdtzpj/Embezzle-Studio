...
- Themes and Settings: `1.5.0` adds persistent system, light, and dark themes. Settings is regrouped around accounts, providers, models, and app information, while provider details centralize Endpoint, key, model, and enabled-state management. Disabling a provider is now a real runtime boundary: it is excluded from model selection, retry, comparison, voice, and project-default targets, and the app refuses to disable the last available provider. The local `1.6.0` candidate adds Settings deep links from chat empty states and model-discovery outcomes and gives model configuration its own Settings entry instead of burying every model action inside provider details.
...
The current stable Latest remains [`v1.5.0`](https://github.com/szdtzpj/Embezzle-Studio/releases/tag/v1.5.0) / Android versionCode 11. It is immutable, non-prerelease, and contains exactly 3 formal assets. `1.6.0` / code 12 is only a locally production-signed candidate: it has not been pushed, tagged, published as a GitHub Release, or exposed as a public APK. Every local candidate must remain distinguishable from an Actions-built formal APK.
...
`adb devices -l` was empty, so no connected-device acceptance is claimed. The task environment did not expose the built-in browser either; therefore the passing Web export is not being presented as browser visual/interaction acceptance. Real OpenAI, Volcengine Ark, Alibaba Bailian, and other provider accounts were not exercised. Public stable Latest remains `v1.5.0`; this round did not push, tag, or create a `v1.6.0` Release. See the [`1.6.0` local-candidate continuation checkpoint](./docs/CONTINUATION_CHECKPOINT_2026-07-13_V1.6.md) for the complete local evidence and publication boundary.
