...
- Themes and Settings: the `1.5.0` candidate adds persistent system, light, and dark themes. Settings is regrouped around accounts, providers, models, and app information, while provider details centralize Endpoint, key, model, and enabled-state management. Disabling a provider is now a real runtime boundary: it is excluded from model selection, retry, comparison, voice, and project-default targets, and the app refuses to disable the last available provider.
...
The final rebuilt D-drive APK will still be local pre-publication evidence rather than a formal GitHub asset, and it will not stand in for a not-yet-created `v1.5.0` tag, Release, or production Android/Pages Actions run. The public stable Latest remains `v1.4.0`. `adb devices -l` remains empty, so no connected-device acceptance of this candidate is claimed; real-provider billing and MCP acceptance also remain external boundaries. See the [`1.5.0` pre-publication continuation checkpoint](./docs/CONTINUATION_CHECKPOINT_2026-07-12_V1.5.md) for the complete current state.
