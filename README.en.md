...
- Encrypted backup and MCP safety: password-protected local exports exclude structured API-key/MCP-authorization fields, media, and the device-local attempt ledger `providerUsageEvents`. Ordinary conversations, prompts, templates, and error text are exported as authored, so secrets must not be pasted into those fields. Android automatic app backup is disabled, so device migration should use the explicit authenticated encrypted export; the clean `1.2.0` prebuild and packaged Manifest both verify `android:allowBackup="false"`. Remote MCP configuration is disabled by default and permission-gated; tool execution remains fail closed until a provider-specific per-call approval loop is complete.
...
The development branch is now `1.2.0` / Android versionCode 8, but it has not been tagged or released and has not replaced public Latest. Public stable remains `v1.0.6`; the existing local `1.1.0` candidate is historical evidence for the previous development phase, not a `1.2.0` APK or a public Release asset.
...
The current development build is `1.2.0` / versionCode 8. Its local quality gate, Web export and 390×844 browser regression, Expo Doctor 20/20, three workflow YAML files, 35 Bash blocks, clean Android prebuild/Release assembly, and production-certificate candidate signing all pass. The candidate APK is `D:\EmbezzleStudio-Releases\v1.2.0-candidate\Embezzle-Studio-v1.2.0-candidate-release.apk`, 97,313,239 bytes, SHA-256 `872f32a48320f2a20dadee6fc0f699668666d067a60e546a19467ed922082da0`. `aapt` and the packaged Manifest verify identity, SDKs, intentional `RECORD_AUDIO`, and `allowBackup=false`; camera and overlay are absent. `apksigner` verifies exactly one expected production signer, v2/v3, and zip alignment. This remains a local candidate: it has not been pushed, tagged, uploaded, or published as a GitHub Release.
...
3. Pause other `main` merges and newer-version Releases. Create and push a tag matching the application version, such as the current development version `v1.2.0`, from the exact latest `origin/main` commit.
