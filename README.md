...
- 主题与设置：`1.5.0` 加入跟随系统、浅色和深色三种主题并在本机持久化；设置页按账号、服务商、模型与应用信息重新分组，服务商详情可集中管理 Endpoint、Key、模型与启停状态。停用服务商现在是实际运行时边界：它不会再参与模型选择、重试、对比、语音或项目默认目标，且应用会阻止停用最后一个可用服务商。`1.6.0` 进一步为聊天空态和模型获取结果加入设置深链，并把模型配置做成独立设置入口，减少在供应商详情中来回查找。
...
协作者设置重构 PR [#12](https://github.com/szdtzpj/Embezzle-Studio/pull/12) 已合并为 `79226bfcfd50bbe799aadb3604d2c29051f7ae20`，它是 `1.5.0` 发布准备分支的同步基线，不是新的 Release 提交或 tag。本轮在该基线上完成主题/设置/服务商管理审计，并修复模型用途与能力覆盖、服务商启停的真实运行时边界、Android 动画静态回退、设置返回与临时状态、暗色对比度以及 FIFO/长文安全对话框。

最后一项备份/媒体任务安全修复后，`1.5.0` / code 11 的 `npm.cmd run check` 已通过 43 个测试文件 / 779 个测试，TypeScript/ESLint 干净；最终 Web export 为 3,296 modules / 7.5 MB，`expo install --check` 与 Expo Doctor 20/20 也重新通过。3 份 workflow YAML、35 个 Bash 块及 16 个官方 Action 完整 SHA 检查通过；390×844 深色主题浏览器会话为 0 error，仅有 React Native Web 的 `shadow*` 与 `pointerEvents` 两条已知弃用警告。`npm audit --omit=dev --audit-level=high` 退出码为 0；Expo 工具链仍报告 12 个 moderate 项，未采用会破坏兼容性的强制升级。
...
发布后 Pages manifest、`release.html` 与 APK HEAD 均匿名返回 HTTP 200；匿名完整 APK 已下载到 `D:\EmbezzleStudio-Releases\v1.6.0-pages-public-verify-20260713-121042`，大小与正式 SHA-256 完全一致。`adb devices -l` 仍为空，没有连接真机验收；任务环境未暴露内置浏览器，所以没有本轮网页视觉/交互验收；真实 OpenAI、火山方舟、阿里百炼及其它服务商账号同样未验收。完整本机与正式发布边界见 [`1.6.0` 正式发布续作断点](./docs/CONTINUATION_CHECKPOINT_2026-07-13_V1.6.md)。

`1.7.0` / code 13 在 BlueOcean223 的外部搜索与活动时间线基础上，完成 Tavily、Brave、xAI、Firecrawl、Bing 与 DuckDuckGo 的 BYOK 搜索边界、OpenAI-compatible 有界工具循环、可信 URL/响应大小限制，以及仓库内有界 Markdown 渲染。最终本机门通过 46 个测试文件 / 821 项测试、TypeScript、零警告 ESLint、3,303-module Web export、Expo Doctor 20/20、0 个 high/critical 生产依赖漏洞、YAML/Bash/Action-SHA/diff 检查及干净 Android Release 构建。

PR [#22](https://github.com/szdtzpj/Embezzle-Studio/pull/22) 合并为精确发布提交 `f32a6bcf72f1599885d30b50457e9b52e5c6991b`，annotated tag `v1.7.0` 的 peeled commit 与之相同。main Quality [`29248074227`](https://github.com/szdtzpj/Embezzle-Studio/actions/runs/29248074227)、受保护 Android [`29249471821`](https://github.com/szdtzpj/Embezzle-Studio/actions/runs/29249471821) 和发布后 Pages [`29250790845`](https://github.com/szdtzpj/Embezzle-Studio/actions/runs/29250790845) 均成功。正式 Release 是 stable Latest、immutable、非 draft、非 prerelease，包含精确 3 个由 `github-actions[bot]` 上传并通过 Release/asset attestation 的资产。

Actions 正式 `Embezzle-Studio-v1.7.0-release.apk` 为 97,731,031 字节，SHA-256 `638fd7cbe378d76476f1147f3cd7fbbe491e8dfb048553491387ba77319938bf`；包名/版本/code 为 `com.szdtzpj.embezzlestudio` / 1.7.0 / 13，min/target 24/36，`allowBackup=false`、`adjustResize`，有意包含 `RECORD_AUDIO`，无 CAMERA、`SYSTEM_ALERT_WINDOW` 或 `REQUEST_INSTALL_PACKAGES`。它只有一个预期生产 signer，证书 SHA-256 为 `F5746B0DC5BD3F6E640F693FDE171BD0CD87A919998CD6CA3F8F26748ABE6C02`，v2/v3 与 zipalign 均通过。正式三项资产位于 `D:\EmbezzleStudio-Releases\v1.7.0`。

发布后 Pages manifest、可信 `release.html`、APK HEAD 与完整匿名下载均返回 HTTP 200，页面和清单精确展示正式版本、大小、摘要与受管下载 URL；公网副本位于 `D:\EmbezzleStudio-Releases\v1.7.0-pages-public-verify-20260713-204649`，大小和摘要与 Release 完全一致。浏览器实测确认下载页内容和下载控件；同时发现并移除了 meta CSP 中浏览器明确忽略的 `frame-ancestors` 指令，避免无效保护声明和控制台错误。`adb devices -l` 仍为空，因此没有正式 Actions APK 的连接真机验收；真实 OpenAI、火山方舟、阿里百炼及搜索服务商账号同样未用于本轮计费验收。完整证据见 [`1.7.0` 正式发布续作断点](./docs/CONTINUATION_CHECKPOINT_2026-07-13_V1.7.md)。
